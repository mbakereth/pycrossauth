...
class InMemoryKeyStorage(KeyStorage):
    def __init__(self):
        ...
        self._keys_by_user_id: Dict[str|int, List[Key]] = {}
        self._non_user_keys: List[Key] = []
    ...
    async def save_key(self, userid: Optional[Union[str, int]], 
        ...
        if userid is not None:
            if userid not in self._keys_by_user_id:
                self._keys_by_user_id[userid] = [key]
            else:
                self._keys_by_user_id[userid].append(key)
        else:
            self._non_user_keys.append(key)
    ...
    async def delete_key(self, value: str) -> None:
        if value in self._keys:
            ...
            if "userid" in key:
                userid = key["userid"]
                if (userid != Null):
                    del self._keys_by_user_id[userid] # type: ignore
            else:
                self._non_user_keys = [v for v in self._non_user_keys if v["value"] != value]
    ...
    async def delete_all_for_user(self, userid: str|int|None, 
        ...
        if userid:
            if userid in self._keys_by_user_id:
                del self._keys_by_user_id[userid]
        else:
            self._non_user_keys = []

    async def get_all_for_user(self, userid: str|int|None = None) -> List[Key]:
        if not userid:
            return self._non_user_keys
        return self._keys_by_user_id.get(userid, [])

    async def delete_matching(self, key: PartialKey) -> None:
        delete_from : List[Key] = self._non_user_keys
        if ("userid" in key and key["userid"] != None):
            if (key["userid"] not in self._keys_by_user_id): return
            delete_from = self._keys_by_user_id[key["userid"]]
        matches : list[Key] = []
        ...
        for match in matches:
            ...
            del self._keys[match["value"]]

    async def update_key(self, key: PartialKey) -> None:
        if 'value' in key and key['value'] in self._keys:
            for field, value in key.items():
                setattr(self._keys[key['value']], field, value)
